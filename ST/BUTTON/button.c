                  ...
                  uint8_t btn_trigger_level)
{
  if( btn == NULL)
  {
    TRACE_ERROR("struct button is null!");
    ASSERT(ASSERT_ERR);
  }
...
void Search_Button(void)
{
  ...
  for(pass_btn = Head_Button; pass_btn != NULL; pass_btn = pass_btn->Next)
  {
    TRACE_INFO("button node have %s",pass_btn->Name);
  }
}
...
void Button_Process_CallBack(void *btn)
{
  ...
  switch(btn_event)
  {
    case BUTTON_DOWM:
    {
      TRACE_INFO("添加你的按下触发的处理逻辑");
      break;
    }
    
    case BUTTON_UP:
    {
      TRACE_INFO("添加你的释放触发的处理逻辑");
      break;
    }
    
    case BUTTON_DOUBLE:
    {
      TRACE_INFO("添加你的双击触发的处理逻辑");
      break;
    }
    
    case BUTTON_LONG:
    {
      TRACE_INFO("添加你的长按触发的处理逻辑");
      break;
    }
    
    case BUTTON_LONG_FREE:
    {
      TRACE_INFO("添加你的长按释放触发的处理逻辑");
      break;
    }
    
    case BUTTON_CONTINUOS:
    {
      TRACE_INFO("添加你的连续触发的处理逻辑");
      break;
    }
    
    case BUTTON_CONTINUOS_FREE:
    {
      TRACE_INFO("添加你的连续触发释放的处理逻辑");
      break;
    }
...
static void Print_Btn_Info(Button_t* btn)
{
  ...
  TRACE_INFO("button struct information:\n\
              btn->Name:%s \n\
              btn->Button_State:%d \n\
