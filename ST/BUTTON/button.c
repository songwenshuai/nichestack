...
void Button_Cycle_Process(Button_t *btn)
{
  ...
  if ((current_level != btn->Button_Last_Level) && (++(btn->Debounce_Time) >= BUTTON_DEBOUNCE_TIME)) //按键电平发生变化，消抖
  {
    ...
    if ((btn->Button_State == NONE_TRIGGER) || (btn->Button_State == BUTTON_DOUBLE))    //如果按键是没被按下的，改变按键状态为按下(首次按下/双击按下)
    {
...
void Search_Button(void)
{
  ...
  for (pass_btn = Head_Button; pass_btn != NULL; pass_btn = pass_btn->Next)
  {
    TRACE_DEBUG("button node have %s\n", pass_btn->Name);
  }
}
...
static void Print_Btn_Info(Button_t *btn)
{
  ...
  TRACE_DEBUG("button struct information:\n\
              btn->Name:%s \n\
              btn->Button_State:%d \n\
