...
int printk_mkfloatstr(double *f,char c[],int precision_width)
{
	...
	q=(uint64_t)(*f-p)*100000000000000000;//小数部分
	else
	{
...
//函数返回：无
//======================================================================
int printk(PRINTK_INFO *info, const char *fmt, _va_list ap)
{
	//Keil 中不支持匿名结构体,需要编译指令#pragma anon_unions指名。
	//#pragma anon_unions
	union ftooc
	{
		double a1;
	...
	for (p = (char *) fmt; (c = *p) != 0; p++)
	{
		...
		case 'i':
			 //此时ap为指向可变参数列表的第一个参数
			ival = (int) _crt_va_arg(ap, int);  //返回当前参数，并将ap指向下一个参数
		...
		case 'f':
			...
			{
			un1.a1 = (double ) _crt_va_arg(ap, double);  //返回当前参数，并将ap指向下一个参数
			temp1=un1.tempc.b8;
			un1.c2=un1.c1;
			...
			{
				ap=ap+4;
				fval= (double)_crt_va_arg(ap,double);
			}
			//确定是否使用小数点精度数值
			if(precision_used)
			{
		...
		case 'x':
		case 'X':
			uval = (unsigned int) _crt_va_arg(ap, unsigned int);
		...
		//输出指针的值
		case 'p':
			uval = (unsigned int) _crt_va_arg(ap, void *);
			vlen = printk_mknumstr(vstr, &uval, FALSE, 16);
			goto cont_u;
		case 'u':
			uval = (unsigned int) _crt_va_arg(ap, unsigned int);
        ...
        //字符型输出
		case 'c':
			cval = (char) _crt_va_arg(ap, unsigned int);
			printk_putc(cval, &count, info);
			break;
		case 's':
			sval = (char *) _crt_va_arg(ap, char *);
			if (sval) {
				vlen = strlen(sval);
				if (!IS_FLAG_MINUS(flags_used)) {
					printk_pad_space(vlen, field_width, &count, info);
...
int myprintf(const char *fmt, ...)
{
	...
	_crt_va_start(ap, fmt);  //对ap 进行初始化，让它指向可变参数表里面的第一个参数
