...
/************************************************************
  ...
  * @note    NULL
  ***********************************************************/
void Button_Create(const char *name, Button_t *btn, unsigned char (*read_btn_level)(void), unsigned char btn_trigger_level)
{
  if (btn == NULL)
  {
...
{
  unsigned char current_level = (unsigned char)btn->Read_Button_Level(); //获取当前按键电平
...
void Search_Button(void)
{
  ...
  for (pass_btn = Head_Button; pass_btn != NULL; pass_btn = pass_btn->Next)
  {
    PRINT_DEBUG("button node have %s", pass_btn->Name);
  }
}
